...
impl Template {
    ...
    pub fn render_with_cache_and_options(
        &self,
        data: &serde_json::Value,
        world_cache: Option<&mut crate::typst::TypstWorld>,
    ) -> Result<crate::render::RenderResult> {
    ...
    pub fn from_file(path: impl AsRef<Path>) -> Result<Self> {
        let content = std::fs::read_to_string(path.as_ref()).map_err(|e| PapermakeError::Io(e))?;
...
impl TemplateBuilder {
    ...
    pub fn content_from_file(mut self, path: impl AsRef<std::path::Path>) -> Result<Self> {
        let content = std::fs::read_to_string(path).map_err(|e| PapermakeError::Io(e))?;
    ...
    pub fn build(self) -> Result<Template> {
        let content = self
            .content
            .ok_or_else(|| PapermakeError::Template("Template content is required".to_string()))?;
        let schema = self.schema.unwrap_or_default();

        Ok(Template { content, schema })
    }
    ...
    pub fn from_raw_content_cached(
        content: impl Into<String>,
    ) -> Result<crate::cache::CachedTemplate> {
        use crate::cache::TemplateCache;
        let template = Self::new().content(content).build()?;
