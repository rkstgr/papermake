...
pub trait RegistryStorage {
    ...
    async fn list_templates_by_scope(&self, scope: &TemplateScope) -> Result<Vec<(papermake::TemplateId, u64)>>;
    
    /// Search templates by name/description
    async fn search_templates(&self, query: &str, user_id: &UserId) -> Result<Vec<(papermake::TemplateId, u64)>>;
    
    /// Check if a user can access a specific template version
    async fn can_user_access(&self, template_id: &papermake::TemplateId, version: u64, user_id: &UserId) -> Result<bool>;
    
    // === Marketplace Operations ===
    
    /// Update marketplace metadata for a template
    async fn update_marketplace_metadata(&self, template_id: &papermake::TemplateId, version: u64, metadata: &MarketplaceMetadata) -> Result<()>;
    
    /// Get marketplace metadata
    async fn get_marketplace_metadata(&self, template_id: &papermake::TemplateId, version: u64) -> Result<MarketplaceMetadata>;
}
...
mod file_storage {
    ...
    impl FileSystemStorage {
        ...
        fn version_file(&self, id: &papermake::TemplateId, version: u64) -> PathBuf {
            self.template_versions_dir(id).join(format!("{}.json", version))
        }
        ...
        fn org_file(&self, id: &OrgId) -> PathBuf {
            self.base_path.join("organizations").join(format!("{}.json", id.0))
        }
    }
    ...
    impl RegistryStorage for FileSystemStorage {
        async fn save_versioned_template(&self, template: &VersionedTemplate) -> Result<()> {
            ...
            fs::write(self.version_file(&template.template.id, template.version), version_json).await?;
        ...
        async fn get_template_asset(&self, template_id: &papermake::TemplateId, path: &str) -> Result<Vec<u8>> {
            let asset_path = self.template_assets_dir(template_id).join(path);
            fs::read(&asset_path).await
                .map_err(|e| RegistryError::Storage(format!("Failed to read asset {}: {}", asset_path.display(), e)))
        }
        ...
        async fn list_template_assets(&self, template_id: &papermake::TemplateId) -> Result<Vec<String>> {
            ...
            self.list_files_recursive(&assets_dir, &assets_dir, &mut assets).await?;
            Ok(assets)
        }
        ...
        async fn list_templates_by_scope(&self, scope: &TemplateScope) -> Result<Vec<(papermake::TemplateId, u64)>> {
            ...
            while let Some(entry) = entries.next_entry().await? {
                if entry.file_type().await?.is_dir() {
                    let template_id = papermake::TemplateId(entry.file_name().to_string_lossy().to_string());
                    let versions = self.list_template_versions(&template_id).await?;
                    
                    for version in versions {
                        if let Ok(versioned_template) = self.get_versioned_template(&template_id, version).await {
        ...
        async fn search_templates(&self, query: &str, _user_id: &UserId) -> Result<Vec<(papermake::TemplateId, u64)>> {
            ...
            while let Some(entry) = entries.next_entry().await? {
                if entry.file_type().await?.is_dir() {
                    ...
                    for version in versions {
                        if let Ok(versioned_template) = self.get_versioned_template(&template_id, version).await {
                            let name_matches = versioned_template.template.name.to_lowercase().contains(&query_lower);
                            let desc_matches = versioned_template.template.description
                                .as_ref()
        ...
        async fn get_marketplace_metadata(&self, template_id: &papermake::TemplateId, version: u64) -> Result<MarketplaceMetadata> {
            ...
            template.marketplace_metadata.ok_or_else(|| {
                RegistryError::Storage(format!("No marketplace metadata for template {} version {}", template_id.as_ref(), version))
            })
        }
    }
    ...
    impl FileSystemStorage {
        ...
        async fn list_files_recursive(&self, dir: &Path, base: &Path, files: &mut Vec<String>) -> Result<()> {
